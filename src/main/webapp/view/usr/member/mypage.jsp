	...
	<table class="mypage-table">
		<tbody>
					...
					<div class="email-auth-wrap">
						<c:choose>
							...
							<c:otherwise>
								...
								<a href="/usr/member/emailAuth"
									class="btn btn-sm btn-neutral ml-2"> 인증하기 </a>
							</c:otherwise>
						</c:choose>
			...
			<tr>
				...
				<td><c:choose>
						<c:when
							test="${sessionScope.loginedMember.memberAuthType == 'LOCAL'}">
                        일반 회원
                    </c:when>
						<c:when
							test="${sessionScope.loginedMember.memberAuthType == 'KAKAO'}">
                        카카오 회원
                    </c:when>
						<c:when
							test="${sessionScope.loginedMember.memberAuthType == 'NAVER'}">
                        네이버 회원
                    </c:when>
						<c:otherwise>
                        구글 회원
			...
			<tr>
				<th>가입일</th>
				<td>${fn:replace(member.regDate, 'T', ' ')}</td>
			</tr>
			...
			<c:if test="${not empty member.lastLoginAt}">
				<tr>
					<th>최근 로그인</th>
					<td>${fn:replace(member.lastLoginAt, 'T', ' ')}</td>
				</tr>
			</c:if>
