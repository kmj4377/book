...
public class UsrExpenseController {
	...
	private Expense loadUserExpenseOrNull(int id) {
		...
		if (expense == null)
			return null;
		if (expense.getMemberId() != req.getLoginedMemberId())
			return null;
		return expense;
	}
	...
	@PostMapping("/doWrite")
	@ResponseBody
	public String doWrite(@RequestParam int amount, @RequestParam(required = false) Integer subCategoryId,
			@RequestParam(required = false) String memo, @RequestParam String expenseDate) {
		if (needLogin())
		...
		expenseService.write(req.getLoginedMemberId(), subCategoryId, amount, memo, expenseDate);

		return req.jsReplace("지출이 등록되었습니다.", "/usr/expense/list");
	}

	@GetMapping("/list")
	public String showList(@RequestParam(required = false) String date, @RequestParam(required = false) String keyword,
			Model model) {
		LoginedMember user = req.getLoginedMember();
		if (user == null)
	...
	@GetMapping("/detail")
	public String showDetail(@RequestParam(required = false) Integer id, @RequestParam(required = false) String date,
			Model model) {
		LoginedMember user = req.getLoginedMember();
		if (user == null)
			return "redirect:/usr/member/login";
	...
	@ResponseBody
	public String doModify(@RequestParam int id, @RequestParam(required = false) Integer subCategoryId,
			@RequestParam int amount, @RequestParam(required = false) String memo, @RequestParam String expenseDate) {
		if (needLogin())
			return req.jsReplace("로그인 후 이용해주세요.", "/usr/member/login");
		...
		expenseService.update(id, subCategoryId, amount, memo, expenseDate);
